...
export default function DayModal({ isOpen, date, events, onClose, onCreate, onEdit, onDelete }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 select-none">
      <div className="w-full max-w-md bg-white rounded-xl shadow-2xl p-6 max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-800">События {dateStr}</h3>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-xl select-none">×</button>
        </div>

        <div className="flex-1 overflow-y-auto space-y-3 pr-1">
          {sorted.length === 0 ? (
            <div className="text-center text-gray-400 py-8">Нет планов на этот день</div>
          ) : (
            sorted.map((ev) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs text-gray-500">{typeLabels[ev.type] || ev.type}</span>
                    <span className="text-xs text-gray-400 ml-auto">{timeLabel}</span>
                  </div>
                  <div className="font-medium text-gray-800 text-sm">{ev.title}</div>
                  {ev.amount && (
                    <div className="text-xs text-orange-600 mt-1 font-medium">{ev.amount.toLocaleString('ru-RU')} ₽</div>
                  )}
                  {ev.description && (
                    <div className="text-xs text-gray-500 mt-1 line-clamp-2">{ev.description}</div>
                  )}
                  {rangeText && (
                    <div className="text-xs text-indigo-600 mt-1 font-medium">{rangeText}</div>
                  )}
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <button
                      onClick={() => onEdit(ev)}
                      className="text-xs px-2 py-1 bg-indigo-50 text-indigo-700 rounded hover:bg-indigo-100 transition select-none"
                    >
                      Редактировать
                    </button>
                    <button
                      ...
                      className="text-xs px-2 py-1 bg-red-50 text-red-600 rounded hover:bg-red-100 transition select-none"
                    >
                      Удалить
                    </button>
                  </div>
        ...
        <div className="pt-4 border-t border-gray-100 mt-4">
          <button
            onClick={onCreate}
            className="w-full py-2 bg-indigo-600 text-white rounded-lg font-medium hover:bg-indigo-700 transition select-none"
          >
            + Добавить
          </button>
        </div>
